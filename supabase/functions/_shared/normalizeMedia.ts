 ...
 *
 * Ported from project/handoff/normalizeMedia.js (the coding-agent handoff
 * written during the design pass) to TypeScript for the Deno edge runtime.
 * Behavior is unchanged; only types were added.
 *
...
export interface MediaItem {
  id: string;              // Stable, provider-prefixed: "tmdb:movie:603"
  title: string;
  mediaType: MediaType;
  source: string;          // Provider id, for attribution
  creator?: string;        // Director, author, artist, publisher
  imageUrl?: string;       // Poster, cover or sleeve
  backdropUrl?: string;    // Wide art, where the provider has it
  releaseYear?: number;
  synopsis?: string;
  rating?: number;         // Normalised to 0–5, never the raw scale
  ratingCount?: number;
  previewUrl?: string;     // 30s audio, where licensed
  externalUrl?: string;    // Open on the provider
  genres?: string[];
  runtimeMins?: number;
}
...
const toFiveScale = (value: unknown, fromMax: number): number | undefined => {
  if (typeof value !== 'number' || Number.isNaN(value)) return undefined;
  return Math.round((value / fromMax) * 5 * 10) / 10;
};

const yearFrom = (dateish: unknown): number | undefined => {
...
export function fromTMDBMovie(r: any): MediaItem | null {
  ...
  return {
    ...
    creator: clean(r.credits?.crew?.find((c: any) => c.job === 'Director')?.name),
    imageUrl: r.poster_path ? `${TMDB_IMG}/w500${r.poster_path}` : undefined,
    backdropUrl: r.backdrop_path ? `${TMDB_IMG}/w1280${r.backdrop_path}` : undefined,
...
export function fromTMDBShow(r: any): MediaItem | null {
  if (!r || !r.id) return null;
  return {
    id: `tmdb:tv:${r.id}`,
    source: 'tmdb',
    ...
    creator: clean(r.created_by?.[0]?.name || r.networks?.[0]?.name),
    imageUrl: r.poster_path ? `${TMDB_IMG}/w500${r.poster_path}` : undefined,
    backdropUrl: r.backdrop_path ? `${TMDB_IMG}/w1280${r.backdrop_path}` : undefined,
...
export function fromSpotifyAlbum(r: any): MediaItem | null {
  if (!r || !r.id) return null;
  /* Spotify returns images largest-first; take the largest under ~800px. */
  const image = (r.images || []).find((i: any) => i.width && i.width <= 800) || r.images?.[0];
  return {
    id: `spotify:album:${r.id}`,
    source: 'spotify',
    mediaType: 'VINYL',
    title: clean(r.name) || 'Untitled',
    creator: clean((r.artists || []).map((a: any) => a.name).join(', ')),
    imageUrl: image?.url,
    releaseYear: yearFrom(r.release_date),
    genres: r.genres || [],
...
export function fromGoogleBook(r: any): MediaItem | null {
  if (!r || !r.id) return null;
  ...
  return {
    ...
    imageUrl: img ? img.replace(/^http:/, 'https:').replace(/&edge=curl/, '') : undefined,
...
export function fromOpenLibraryBook(r: any): MediaItem | null {
  ...
  return {
    ...
    imageUrl: r.cover_i ? `https://covers.openlibrary.org/b/id/${r.cover_i}-L.jpg` : undefined,
...
export function fromAppleResult(r: any): MediaItem | null {
  if (!r) return null;
  const isPodcast = r.kind === 'podcast' || r.wrapperType === 'track'
    ? r.kind === 'podcast'
    : r.collectionType === 'Podcast';
  const type: MediaType = isPodcast ? 'CAST' : 'VINYL';
  ...
  return {
    ...
    runtimeMins: r.trackTimeMillis ? Math.round(r.trackTimeMillis / 60000) : undefined,
  };
}
...
const ADAPTERS: Record<ProviderKind, (r: any) => MediaItem | null> = {
...
export function normalizeMedia(raw: any, kind: ProviderKind): MediaItem | null {
...
export function normalizeList(rawList: any[] | undefined, kind: ProviderKind): MediaItem[] {
  return (rawList || [])
 ...
 */
export function mergeResults(lists: MediaItem[][]): MediaItem[] {
...
export async function settleProviders(
  jobs: Record<string, Promise<MediaItem[]>>
): Promise<{ items: MediaItem[]; failed: string[] }> {
